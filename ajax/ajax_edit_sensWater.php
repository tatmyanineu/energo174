...
WHERE
  "Tepl"."Sensor_cnt".prp_id  =' . $_POST['prp_id'] . '');
    ...
    if ($s_id != '') {
        ...
        $prop_id = pg_fetch_result($sql_prop_id2, 0, 0);
        if ($prop_id != '') {
            if ($date_pov != "") {
                $sql_update_number = pg_query('UPDATE
                "Tepl"."Sensor_Property"
            ...
            if ($date_pov != "") {
                ...
                $prop_id = pg_fetch_result($sql_max_prop_id, 0, 0);
                $prop_id = $prop_id + 1;
                $sql_add_sens_numb = pg_query('INSERT INTO "Tepl"."Sensor_Property" VALUES (' . $prop_id . ', \'' . $date_pov . '\', ' . $s_id . ',2)');
            }
        }
    } else {
    ...
    ');
        ...
        $sql1 = "INSERT";
        $sql_add_sens = pg_query('INSERT INTO  "Tepl"."Sensor_cnt" VALUES (' . $s_id . ', ' . $sen_id . ', \'false\', \'' . $place . '\', ' . $prp_id . ')');
...
$id_log = $id_log + 1;
$sql_add_log = pg_query('INSERT INTO  "public"."logs" VALUES (' . $id_log . ',' . $_POST['plc_id'] . ', \'' . $_SESSION['login'] . '\' , ' . $prp_id . ', \'' . preg_replace('/[;"\']/', '', $diametr) . '\', \'' . $sen_id . '\', \'' . $place . '\', \'' . $number . '\', \'' . $sql1 . '\' , \'' . date("Y-m-d") . '\')');
