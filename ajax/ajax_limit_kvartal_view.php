...
switch ($kvartal) {
    case 101:
        $month = array(1, 2, 3);
        $num = cal_days_in_month(CAL_GREGORIAN, 3, $_POST['year']);
        $date1 = date('' . $_POST['year'] . '-01-01');
        $date2 = date('' . $_POST['year'] . '-03-' . $num);
        $date_now = $date2;
        break;
    case 102:
        $month = array(4, 5, 6);
        $num = cal_days_in_month(CAL_GREGORIAN, 6, $_POST['year']);
        $date1 = date('' . $_POST['year'] . '-04-01');
        $date2 = date('' . $_POST['year'] . '-06-' . $num);
        $date_now = $date2;
        break;
    case 103:
        $month = array(7, 8, 9);
        $num = cal_days_in_month(CAL_GREGORIAN, 9, $_POST['year']);
        $date1 = date('' . $_POST['year'] . '-07-01');
        $date2 = date('' . $_POST['year'] . '-09-' . $num);
        $date_now = $date2;
        break;
    case 104:
        $month = array(10, 11, 12);
        $num = cal_days_in_month(CAL_GREGORIAN, 12, $_POST['year']);
        $date1 = date('' . $_POST['year'] . '-10-01');
        $date2 = date('' . $_POST['year'] . '-12-' . $num);
        $date_now = $date2;
        break;
}
...
echo "<table id='main_table' class='table table-bordered table-condensed'>
        <thead id='thead'>
        <tr id='warning'>
            ...
            <td rowspan=2><b>Тепло</b</td>
            <td rowspan=2><b>Вода</b</td>
          </tr>  <tr id='warning'>
...
function echo_data($id, $max_date, $teplo, $val, $days_v, $days_t) {
    global $arr_all_limit, $date_now, $limit_teplo_part, $limit_voda_part, $corect, $not_alarm, $num, $gr_id, $gr_counter, $gr_row, $month, $kvartal;
    ...
    $td = "<td class='text-center'><a class='btn btn-default btn-sm' id='link' href ='export/limit_kvartal_export.php?id_object=" . $id . "&kvartal=" . $_POST['month'] . "&year=" . $_POST['year'] . "'>Скачать</a></td>";
    $td_voda = "<td  class='text-center'><a class='btn btn-default btn-sm' id='link' href ='export/limit_kvartal_voda_export.php?id_object=" . $id . "&kvartal=" . $_POST['month'] . "&year=" . $_POST['year'] . "'>Скачать</a></td>";
    if ($gr_id != NULL) {
    ...
    if ($key_limit !== false) {
        ...
        for ($l = 0; $l < count($month); $l++) {
            ...
            $num += cal_days_in_month(CAL_GREGORIAN, $month[$l], $_POST['year']);
        }
        ...
        if ($v != FALSE) {
            ...
            if ($gr_id != NULL) {
                if ($gr_counter == 1) {
                    echo "<td  rowspan=" . $gr_row . "> " . str_replace('.', ',', $lim2) . " </td>";
                     ...
                     echo '<td class="text-center" rowspan=' . $gr_row . '><div class="btn-group">
                        <button class="btn btn-default btn-sm  dropdown-toggle" type="button" data-toggle="dropdown">
                       Скачать <span class="caret"></span>
                       </button>
                        <ul class="dropdown-menu">
                          <li><a href="export/exp_group_limit_teplo.php?id_object=' . $gr_id . '&kvartal=' . $_POST['month'] . '&year=' . $_POST['year'] . '">Группа</a></li>';
                        ...
                        $q = 1;
                        while ($gr_rows = pg_fetch_row($sql_plc_group)) {
                            echo "<li><a href='export/limit_kvartal_export.php?id_object=" . $gr_rows[0] . "&kvartal=" . $_POST['month'] . "&year=" . $_POST['year'] . "'> Филиал " . $q . "</a></li>";
                            $q++;
                        }
                        echo '</ul>
                      </div>
                      ...
                      ';
                       ...
                       Скачать <span class="caret"></span>
                        ...
                        <ul class="dropdown-menu">
                          <li><a href="export/exp_group_limit_voda.php?id_object=' . $gr_id . '&kvartal=' . $_POST['month'] . '&year=' . $_POST['year'] . '">Группа</a></li>';
                        $q = 1;
                         $sql_plc_group = pg_query('SELECT 
                                public.group_plc.plc_id
                              FROM
                              ...
                              WHERE
                                public.group_plc.group_id=' . $gr_id);
                        while ($gr_row = pg_fetch_row($sql_plc_group)) {
                            echo "<li><a href='export/limit_kvartal_voda_export.php?id_object=" . $gr_rows[0] . "&kvartal=" . $_POST['month'] . "&year=" . $_POST['year'] . "'> Филиал " . $q . "</a></li>";
                            $q++;
                        }
                        echo '</ul>
                      </div>
            ...
            if ($val == 0) {
                ...
                if ($gr_id != NULL) {
                    if ($gr_counter == 1) {
                        echo "<td  rowspan=" . $gr_row . "> " . str_replace('.', ',', $lim2) . " </td>";
//                        echo "<td><a id='link' href ='export/exp_group_limit_teplo.php?id_object=" . $gr_id . "&kvartal=" . $_POST['month'] . "&year=" . $_POST['year'] . "'>Группа</a></td>";
//                        echo "<td><a id='link' href ='export/exp_group_limit_voda.php?id_object=" . $gr_id . "&kvartal=" . $_POST['month'] . "&year=" . $_POST['year'] . "'>Группа</a></td>";
                        echo '<td class="text-center" rowspan=' . $gr_row . '><div class="btn-group">
                        <button class="btn btn-default btn-sm  dropdown-toggle" type="button" data-toggle="dropdown">
                       Скачать <span class="caret"></span>
                       </button>
