...
if (pg_num_rows($sql_all_limit) > 0) {
    ...
    $sql_in_group = pg_query('SELECT DISTINCT 
        public.group_plc.group_id
      FROM
        public.group_plc
      WHERE
        public.group_plc.plc_id = ' . $_GET['id_object'] . '');
    
    $id_group = pg_fetch_result($sql_in_group, 0, 0);
    
    $sql_plc_in_group = pg_query('SELECT DISTINCT 
                        public.group_plc.plc_id,
                        public."LimitPlaces_cnt".teplo,
                        public."LimitPlaces_cnt".voda
                      FROM
                        public."LimitPlaces_cnt"
                        INNER JOIN public.group_plc ON (public."LimitPlaces_cnt".plc_id = public.group_plc.group_id)
                      WHERE
                        public.group_plc.group_id = ' . $id_group . '');
    $count = pg_num_rows($sql_plc_in_group);
    $tep = pg_fetch_result($sql_plc_in_group, 0, 1)/$count;
    $vod =pg_fetch_result($sql_plc_in_group, 0, 2)/$count;
    
     $arr_all_limit[] = array(
            'plc_id' => $_GET['id_object'],
            'teplo' => $tep,
            'voda' => $vod
        );
}
