...
$date1 = date('Y-m-25', strtotime('-1 month')); //подогнать формат даты по yyyy-mm-dd hh:mm:ss TZD
$date2 = date('Y-m-26');
...
$dom->save('archive/export_' . $date1 . '-' . date('Y-m-d', strtotime('-1 day' , strtotime($date2))) . '.xml'); // сохранение файла 
$file = 'export_' . $date1 . '-' . date('Y-m-d', strtotime('-1 day' , strtotime($date2))) . '.xml';
...
if (!isset($_POST['date1']) and ! isset($_POST['date2'])) {
  ...
  FROM mail_settings');
    ...
    $text = "<h2>Добрый день направляем вам файл за период с ". date("d.m.Y", strtotime($date1))." по ". date("d.m.Y", strtotime('-1 day' , strtotime($date2)))." <br>для получения файла перейдите по  <a href=\"http://energofond174.ru:8060/fortum/download_xml.php?file=".$file."\">ссылке</a></h2> ";
...
//$header .= "Content-Type: multipart/mixed; boundary=\"----------A4D921C2D10D7DB\"\r\n";
    ...
    $subject = 'Месячная выгрузка для УТСК показаний потребления тепловой энергии за период с ' . date("d.m.Y", strtotime($date1)) . ' по '.date("d.m.Y", strtotime('-1 day' , strtotime($date2))) .', по муниципальным учреждениям г. Челябинска';
    $header .= "From: =?windows-1251?Q?" . str_replace("+", "_", str_replace("%", "=", urlencode('' . $arr['mail'] . ''))) . "?= <" . $arr['mail'] . ">\r\n";
    $header .= "Content-Type:  text/html; boundary=\"----------A4D921C2D10D7DB\"\r\n";
...
//    $header .= "Content-Type:  text/html; boundary=\"----------A4D921C2D10D7DB\"\r\n";
    ...
    if ($result === true) {
        ...
        echo "Письмо не отправлено. Ошибка: " .iconv('utf8', 'cp1251', $result) . "</br>";
    }
}