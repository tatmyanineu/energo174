...
$type_arch = 2;
$sql_error = pg_query('SELECT * FROM fault_cnt WHERE id =1');
...
//var_dump($array_pogr);
    ...
    $id_list[] = array();
    $sql_incident = pg_query('SELECT plc_id FROM fault_inc WHERE numb = 1 ORDER BY plc_id');
    ...
    for ($i = 0; $i < count($array_pogr); $i++) {
        ...
        if ($k === false) {
            ...
            $text_error = 'Погрешность => ((Подача (t1: ' . number_format($array_pogr[$i]['t1'], 2, '.', '') . ') / Обратка (t2: ' . number_format($array_pogr[$i]['t2'], 2, '.', '') . ') )*100)-100 = ' . number_format($array_pogr[$i]['t'], 2, '.', '') . '; <br> Погреш. (' . number_format($array_pogr[$i]['t'], 2, '.', '') . ') > Допуст. Погр.(' . $pogr . ')';
    ...
    $sql = pg_query('UPDATE fault_cnt SET date_time=\'' . date('Y-m-d H:i:00') . '\' WHERE id=1');
    echo "</tbody></table>";
}
?>